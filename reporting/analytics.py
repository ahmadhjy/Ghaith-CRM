...
def _lines_cost_usd(date_from=None, date_to=None, *, sales_employee_id=None, destination_id=None):
    ...
    if sales_employee_id:
        qs = qs.filter(line_employee_id=sales_employee_id)
    if destination_id:
...
def build_dashboard_analytics(request):
    ...
    today = date.today()
    sales_employee_id = (request.GET.get('sales_employee') or '').strip()
    ...
    if sales_employee_id:
        inv_q = inv_q.filter(sales_employee_id=sales_employee_id)
    if destination_id:
        inv_q = inv_q.filter(main_destination_id=destination_id)

    revenue = inv_q.aggregate(t=Sum("grand_total_usd"))["t"] or Decimal("0.00")
    cogs = _lines_cost_usd(
        date_from, date_to,
        sales_employee_id=sales_employee_id or None,
    ...
    for emp in Employee.objects.filter(role=Employee.EmployeeRole.SALES).order_by("name"):
        if sales_employee_id and str(emp.id) != sales_employee_id:
            continue
        ...
        if destination_id:
            lines = lines.filter(destination_id=destination_id)
        sales = Decimal("0.00")
        cost = Decimal("0.00")
        for line in lines:
    ...
    return {
        ...
        "filter_sales_employee": sales_employee_id,
        "filter_destination": destination_id,
    }
