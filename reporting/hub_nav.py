...
ACCOUNTING_HOME = '/accounting/'


def _accounting_path(request):
    """Path relative to /accounting/ (e.g. '' or '/reporting/ar-aging')."""
    path = (request.path if request else '/accounting/').rstrip('/')
    if path.startswith('/accounting'):
        rel = path[len('/accounting'):] or '/'
...
def _home_url(request, tab=None):
    ...
    if tab:
        ...
        params.pop('tab', None)
    qs = params.urlencode()
    return ACCOUNTING_HOME + ('?' + qs if qs else '')


def resolve_hub_section(request):
    path = _accounting_path(request)
...
def build_hub_nav(request):
    ...
    if section == 'receivables':
        sub_tabs = [
            ...
            {'id': 'clients', 'label': 'Client list', 'url': reverse('accounts_core:clients_list'), 'active': False},
        ]
        ...
        sub_tabs = [
            ...
            {'id': 'suppliers', 'label': 'Supplier list', 'url': reverse('accounts_core:suppliers_list'), 'active': False},
        ]
    elif section == 'cash':
        sub_tabs = [
