...
class SupplierStatementServiceDateTests(TestCase):
    ...
    def test_unissued_line_excluded_from_supplier_statement(self):
        inv = SalesInvoice.objects.create(
            invoice_no="TMP-UNISS",
            client=self.client,
            sales_employee=self.employee,
            issue_date=date.today(),
            currency="USD",
...
class OpeningBalanceStatementTests(TestCase):
    """Opening balance row on SOA when period starts before as_of (go-live) date."""

    def setUp(self):
        from accounting_bridge.models import PartyOpeningBalance

        self.client_obj = Client.objects.create(client_code="C-OB", name_en="Opening Client")
        self.supplier = Supplier.objects.create(
            supplier_code="S-OB", name="Opening Supplier", managing_number="+96170000001"
        )
        self.go_live = date(2026, 6, 24)
        PartyOpeningBalance.objects.create(
            party_type=PartyOpeningBalance.PartyType.CLIENT,
            client=self.client_obj,
            as_of_date=self.go_live,
            debit_usd=Decimal("1500.00"),
            credit_usd=Decimal("200.00"),
        )
        PartyOpeningBalance.objects.create(
            party_type=PartyOpeningBalance.PartyType.SUPPLIER,
            supplier=self.supplier,
            as_of_date=self.go_live,
            debit_usd=Decimal("100.00"),
            credit_usd=Decimal("800.00"),
        )

    def test_client_opening_when_date_from_before_as_of(self):
        rows = build_client_statement_rows(
            self.client_obj,
            date_from=date(2026, 1, 1),
            date_to=date(2026, 12, 31),
        )
        ...
        self.assertEqual(opening[0]["debit"], Decimal("1500.00"))
        self.assertEqual(opening[0]["credit"], Decimal("200.00"))
        self.assertEqual(opening[0]["ref"], "OPEN")

    def test_supplier_opening_when_date_from_before_as_of(self):
        rows = build_supplier_statement_rows(
            self.supplier,
            date_from=date(2026, 1, 1),
            date_to=date(2026, 12, 31),
        )
        opening = [r for r in rows if r.get("sort_id") == "opening"]
        self.assertEqual(len(opening), 1)
        self.assertEqual(opening[0]["debit"], Decimal("100.00"))
        self.assertEqual(opening[0]["credit"], Decimal("800.00"))

    def test_opening_excluded_when_period_ends_before_as_of(self):
        rows = build_client_statement_rows(
            self.client_obj,
            date_from=date(2026, 1, 1),
