...
class ToggleCheckbox(forms.CheckboxInput):
    """CRM-style toggle switch for boolean line fields."""

    def render(self, name, value, attrs=None, renderer=None):
        final_attrs = self.build_attrs(attrs or {}, extra_attrs={'type': 'checkbox', 'name': name})
        input_id = final_attrs.pop('id', name)
        css_class = final_attrs.pop('class', '')
        checked = ' checked' if value else ''
        extra = ''
        for k, v in final_attrs.items():
            extra += f' {k}="{v}"'
        return mark_safe(
            f'<label class="inv-toggle-label"><span class="inv-toggle">'
            f'<input type="checkbox" name="{name}" id="{input_id}" class="{css_class}"{checked}{extra}>'
            f'<span class="inv-toggle__slider"></span></span></label>'
        )
...
class SalesInvoiceLineBaseForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "notes": forms.Textarea(attrs={"rows": 3, "class": "line-details-field", "placeholder": "Service details…"}),
            "sell_price": forms.NumberInput(attrs={"step": "0.01", "min": "0", "placeholder": "0.00"}),
            "cost_price": forms.NumberInput(attrs={"step": "0.01", "min": "0", "placeholder": "0.00", "class": "line-net-field"}),
            ...
            "send_to_client": ToggleCheckbox(attrs={"class": "line-send-to-client"}),
            "crm_issued": ToggleCheckbox(attrs={"class": "line-crm-issued"}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.fields.get("sell_price"):
            self.fields["sell_price"].label = "Selling"
        if self.fields.get("service_date"):
            self.fields["service_date"].label = "Due"
        if self.fields.get("send_to_client"):
            self.fields["send_to_client"].label = "Sent to client"
        if self.fields.get("crm_issued"):
            self.fields["crm_issued"].label = "Issued"
