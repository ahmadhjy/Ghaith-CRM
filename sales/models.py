...
class SalesInvoiceLine(models.Model):
    ...
    def statement_line_details(self):
        """Details text for statements — CRM notes field, not per-service custom fields."""
        notes = (self.notes or '').strip()
        if notes:
            return notes
        data = self.line_data if isinstance(self.line_data, dict) else {}
        legacy = (data.get('details') or '').strip()
        if legacy:
            return legacy
        st = self.service_type
        if not st and self.service_instance_id:
